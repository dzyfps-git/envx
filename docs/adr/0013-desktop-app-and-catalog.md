...
Status: accepted; engine part from envx 1.5.0, app and packs in later milestones

## Context
envx was usable only by building it from source and setting it up on the command line. To share it, it becomes a
...
1. **Recipes, not redistributed content.** Minecraft's EULA and most mods' licenses ("All Rights Reserved") forbid
   ...
   CurseForge), sha256 hashes and sizes. Each user's envx downloads from those sources, verifies every file against
   the recipe and builds the index locally. An index is only ever built from exactly the supported files.
2. **Supported baselines are built in.** `dev.envx.catalog.Baselines` lists what this build can index (today
   `fabric-1.20.1`, Yarn build.10). The published catalog may list more; one this build does not know is shown as
   needing a newer envx.
3. **Nothing implicit.** A baseline is downloaded only by `envx catalog install <id>` (or the app). A sync whose
...
4. **Own servers only on a supported baseline.** Connecting one's own server stays optional. envx reads what the
...
5. **`envx app-server`**: the desktop app's connection to the engine, JSON lines like `envx api` (`dev.envx.app`),
   versioned separately (`app:1`) because it changes with the app. Long operations stream progress events; writes run
   as separate envx processes as on the command line (ADR 0002). The public `envx api` v1 is unchanged.
...
## Consequences
- The command line keeps working for everyone; `envx init` installs the default baseline.
