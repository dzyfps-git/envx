...
- envx gave more complete answers on pack-wide questions.
...
Isolating the baseline was also fragile. A leftover envx block in CLAUDE.md contaminated one run, because Codex
reads CLAUDE.md too.
...
1. **Local call log.** Every tool call, MCP or CLI, appends one JSON line to `<home>/logs/calls-YYYY-MM.jsonl`:
   ...
   one small append. `bench/extract_tokens.py` reports per-session tokens, model requests and envx/shell calls
   from Codex's and Claude Code's local stores, and summarizes the call log.
2. **`envx agents on|off|status`** switches Codex (`enabled`), the Claude Code registration and the instruction
   ...
   - `setup` installs into `app/<version>` and repoints the registrations **without changing** whether each tool
     is on or off.
   - Files that envx created are deleted again when the block was their only content. The user's own text is always kept.
