...
Status: accepted (2026-09-24, Sevli 0.3.0)

## Context
Questions about past pack versions (benchmark Q10, Q12) had no answer: Sevli indexed only the live server.
...
modpack, and wants Sevli to become public and cross-platform. So history cannot depend on an OS scheduler or
...
3. **Labels and pack identity come from files packs already ship:** `config/bcc.json` (BetterCompatibilityChecker:
   ...
   stays in history.
4. **`sevli env import <env> <folder> [--label]`** adds a past version from a server folder (e.g. a full backup).
...
6. **Use-time sync instead of OS scheduling.** When an MCP session starts, or a CLI query runs, and the environment
   was last checked more than `autoSyncHours` ago (default 6, 0 = off), Sevli starts a separate background
   `env sync --auto`.
   ...
   - OS schedulers stay an optional extra (they can run `sevli env sync <env>`).
   - Limit: a version is lost only if the server changes twice with no Sevli use and no backup in between.
     Sevli never runs on or writes to the server.
...
- Schema v2 is applied by `sevli setup` and by any sync; read-only processes never migrate.
...
in `env`. Both Sevli runs got empty results on the current server and went to GitHub, and one checked
...
- Validated by replaying the 31 recorded Sevli calls of both Q12 Sevli runs: 14 of the 15 empty results now point
