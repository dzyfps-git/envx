...
/**
 * The baselines (Minecraft version + loader + mappings) this version of sevli can index. The published catalog lists
 * baselines too; one this build does not know is shown as needing a newer sevli. A baseline is only downloaded when a
...
public final class Baselines {
    ...
    /** The baseline an environment definition needs, if this sevli supports it. */
    ...
    public static String missing(Config.EnvDef def) {
        return of(def).map(b -> "needs the baseline " + b.name() + ", which is not installed: install it in the app, or run sevli add " + b.id())
