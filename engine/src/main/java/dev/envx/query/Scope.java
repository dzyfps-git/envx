...
public record Scope(String env, long snapshotId, String takenAt, String source, boolean loaderList, Config.EnvDef def,
    ...
    public static Scope resolve(Config config, Db db, String envArg, Path cwd) throws SQLException {
        ...
        if (env == null) {
            throw new IllegalArgumentException("No environment for " + cwd + ". Link it with `envx link <dir> <env>` "
        ...
        if (rows.isEmpty()) throw new IllegalArgumentException("Environment '" + env + "' was never synced. Run `envx env sync " + env + "`.");
