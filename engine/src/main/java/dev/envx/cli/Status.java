...
/** {@code envx status}: everything a person checks day to day, on one screen. Reads only. */
final class Status {
    static final String EVERYDAY = """
            Everyday
              envx status        this screen
              envx on | off      switch envx on or off for Codex and Claude Code (new sessions)
              envx sync [<env>]  pull the server's mods and configs now (it also runs by itself when envx is used)
    ...
    static int print(Config config) throws Exception {
        System.out.println("envx " + Version.VALUE + "  (data home " + config.home() + ")");
        String agents;
        try {
        ...
        if (config.environments.isEmpty()) {
            ...
            try (Db db = Db.open(config.home(), true)) {
                for (String env : config.environments.keySet()) {
                    ...
                    if (dev.envx.env.AutoSync.running(config.home(), env)) System.out.println("  a sync is running");
                    try {
    ...
    private static String environment(Config config, Db db, String env) throws Exception {
        ...
        if (rows.isEmpty()) return "never synced (envx sync " + env + ")";
